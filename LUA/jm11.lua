...
function luaInitJM11(this)
	...
	Mission.TempTbl = {}
	Mission.PowerUpTbl = {"divebomb1","improved_shells","divebomb2"}
...
-- RELEASE_LOGOFF  			luaLog("Cserelodik a "..unit.Class.Name)
			...
			for i,unl in pairs(Mission.Unlockables) do
				--luaLog("EZ AZ UNLOCKID: "..tostring(i).." "..type(i))

-- RELEASE_LOGOFF  				luaLog(tostring(id).." "..type(id))
-- RELEASE_LOGOFF  				luaLog(tostring(i).." "..type(i))

				if tostring(id) == i then
-- RELEASE_LOGOFF  					luaLog("UNLOCK GUINAME ALLITAS: "..tostring(id).." "..unl[1])
-- RELEASE_LOGOFF  					luaLog("breaking")
					SetGuiName(unit, unl[1])
					table.remove(unl, 1)
					break
				else
