...
function luaObj_DoScoring()
	...
	if not Mission.Multiplayer then
		...
		-- RELEASE_LOGOFF  					luaDoCustomLog("luaObj_scoring_"..Mission.Name..".log", "  objective ID: "..tostring(Mission.Objectives.primary[objindex].ID).." | objective.ScoreFailed: "..tostring(Mission.Objectives.primary[objindex].ScoreFailed))
				...
				local currentplayerscore = 1
				for slotID, value in pairs (MPlayers) do
					currentplayerscore = Scoring_GetTotalMissionScore(slotID)
					if currentplayerscore > highestinsteadpointlimit then
						highestinsteadpointlimit = currentplayerscore
					end
				end
				for slotID, value in pairs (MPlayers) do
					actplayerscore = Scoring_GetTotalMissionScore(slotID)
					...
					if actplayerscore > 0 then
						Mission.Objectives.primary[objindex].ScoreFailed = ( Mission.Objectives.primary[objindex].ScoreCompleted / highestinsteadpointlimit ) * actplayerscore
					else
						Mission.Objectives.primary[objindex].ScoreFailed = 0
					end
					if Mission.Objectives.primary[objindex].ScoreFailed ~= 0 then
...
function luaSetMultiMessages(quit, clientsQuit, competitiveClientQuit)
	...
	elseif LobbySettings.GameMode == "globals.gamemode_islandcapture" then
		...
		local counterA = 0
		local counterJ = 0
		--if Mission.CommandBuildings and table.getn(luaRemoveDeadsFromTable(Mission.CommandBuildings)) > 0 then
...
function luaCheckMusic(unit, dist, customMusic)
	...
	if next(planes) ==  nil and next(ships) == nil then	-- ha nincs senki a kozelben, calm
		...
		--if nearestDist and not nearestDist == nil then
...
function luaInitNewSystems()
	
	Mission.Realistic = false
	
	DoFile("gamemode.lua") -- Getting the gamemode from "gamemode.lua"
	
		--if not NewSystemsStarted then
			
			---- SMOKESCREEN SYSTEM -----
			
			--smokeScreens = {}
			--smokeScreenPositions = {}
			Mission.UnitToSmoke = nil
			Mission.SmokeToRemove = nil
			Mission.CurrentSmokeDuration = nil
			Mission.ShipToReEnableSmoke = nil
			Mission.ShipSmokeToRemove = nil
			
			Mission.ActiveSmokes = {}
			Mission.ActiveSmokeRadii = {}
			Mission.ActiveSmokeCoords = {}
			Mission.ShipsInSmoke = {}
			Mission.SmokedShips = {}
			
	if GameMode == 1 then -- Realistic
		
		Mission.Realistic = true
			
			---- LINE OF SIGHTS SYSTEM ----
			
			--Mission.USUnitToSight = nil
			--Mission.JapUnitToSight = nil
			
			---- NEW SYSTEM THINK -----
			
			
			
		--	NewSystemsStarted = true
		
		--end
	
	end
	
	if Mission.Realistic and not Mission.NewSystemsStarted then
		
		if not IsListenerActive("input", "smokeScreenInputListener") then
		
			luaAddSmokeScreenListener()
		
		end
		
		CreateScript("luaInitNewSystemsScript")
	
		Mission.NewSystemsStarted = true
	
	end
	
	--[[if Mission.Classic then
	
		if not IsListenerActive("input", "Launch_Airbase_Stock_1") then
		
			AddListener("input", "Launch_Airbase_Stock_1", {
				["callback"] = "luaABStock1Launched",
				["inputID"] = {MP_LAUNCH_ABSTOCK_1},
			})
			
		end
		
		if not IsListenerActive("input", "Launch_Airbase_Stock_2") then
		
			AddListener("input", "Launch_Airbase_Stock_2", {
				["callback"] = "luaABStock2Launched",
				["inputID"] = {MP_LAUNCH_ABSTOCK_2},
			})
			
		end
		
		if not IsListenerActive("input", "Launch_Airbase_Stock_3") then
		
			AddListener("input", "Launch_Airbase_Stock_3", {
				["callback"] = "luaABStock3Launched",
				["inputID"] = {MP_LAUNCH_ABSTOCK_3},
			})
			
		end
		
		if not IsListenerActive("input", "Launch_Airbase_Stock_4") then
		
			AddListener("input", "Launch_Airbase_Stock_4", {
				["callback"] = "luaABStock4Launched",
				["inputID"] = {MP_LAUNCH_ABSTOCK_4},
			})
			
		end
		
		if not IsListenerActive("input", "Launch_Airbase_Stock_5") then
		
			AddListener("input", "Launch_Airbase_Stock_5", {
				["callback"] = "luaABStock5Launched",
				["inputID"] = {MP_LAUNCH_ABSTOCK_5},
			})
			
		end
		
		if not IsListenerActive("input", "Launch_Airbase_Stock_6") then
		
			AddListener("input", "Launch_Airbase_Stock_6", {
				["callback"] = "luaABStock6Launched",
				["inputID"] = {MP_LAUNCH_ABSTOCK_6},
			})
			
		end
		
		if not IsListenerActive("input", "Launch_Airbase_Stock_7") then
		
			AddListener("input", "Launch_Airbase_Stock_7", {
				["callback"] = "luaABStock7Launched",
				["inputID"] = {MP_LAUNCH_ABSTOCK_7},
			})
			
		end
		
		if not IsListenerActive("input", "Launch_Airbase_Stock_8") then
		
			AddListener("input", "Launch_Airbase_Stock_8", {
				["callback"] = "luaABStock8Launched",
				["inputID"] = {MP_LAUNCH_ABSTOCK_8},
			})
			
		end
		
		MissionNarrative("bsm classic listeners added")
...
function luaInitNewSystemsScript(this)
	
	SetThink(this, "luaNewSystemsThink")

end

function luaNewSystemsThink(this, msg)
	
	--MissionNarrative("smoke is thinkin!")
	
	---- LINE OF SIGHT SYSTEM ----
	
	--[[local checkDistance = 9000000
	
	local usShips = luaGetShipsAroundCoordinate({["x"] = 0, ["y"] = 0, ["z"] = 0}, checkDistance, PARTY_ALLIED, "own")
	local usPlanes = luaGetPlanesAroundCoordinate({["x"] = 0, ["y"] = 0, ["z"] = 0}, checkDistance, PARTY_ALLIED, "own")
	local japShips = luaGetShipsAroundCoordinate({["x"] = 0, ["y"] = 0, ["z"] = 0}, checkDistance, PARTY_JAPANESE, "own")
	local japPlanes = luaGetPlanesAroundCoordinate({["x"] = 0, ["y"] = 0, ["z"] = 0}, checkDistance, PARTY_JAPANESE, "own")
	
	local usUnits = luaSumTablesIndex(usShips, usPlanes)
	local japUnits = luaSumTablesIndex(japShips, japPlanes)
	
	usUnits = luaRemoveDeadsFromTable(usUnits)
	japUnits = luaRemoveDeadsFromTable(japUnits)
	
	for usIdx,usUnit in pairs(luaRemoveDeadsFromTable(usUnits)) do
		
		for japIdx,japUnit in pairs(luaRemoveDeadsFromTable(japUnits)) do
			
			usUnit.sighted = false
			
			AddLineOfSightTrigger(japUnit, nil, "luaUSUnitSighted", usUnit, checkDistance)
	...
	for japIdx,japUnit in pairs(luaRemoveDeadsFromTable(japUnits)) do
		...
		for usIdx,usUnit in pairs(luaRemoveDeadsFromTable(usUnits)) do
			...
			japUnit.sighted = false
			
			AddLineOfSightTrigger(usUnit, nil, "luaJapUnitSighted", japUnit, checkDistance)
	...
	for usIdx,usUnit in pairs(luaRemoveDeadsFromTable(usUnits)) do
		...
		local unitParty = usUnit.Party
		local invParty = nil
		
		if unitParty == PARTY_ALLIED then
		
			invParty = PARTY_JAPANESE
		
		elseif unitParty == PARTY_JAPANESE then
		
			invParty = PARTY_ALLIED
		
		end
		
		if not usUnit.sighted then
		
			SetForcedReconLevel(usUnit, 0, invParty)
			--SetForcedReconLevel(usUnit, 0, unitParty)
			
		elseif usUnit.sighted and not usUnit.smoked then
		
			ClearForcedReconLevel(usUnit, invParty)
	...
	for japIdx,japUnit in pairs(luaRemoveDeadsFromTable(japUnits)) do
		...
		if not japUnit.sighted then
		
			SetForcedReconLevel(japUnit, 0, invParty)
			--SetForcedReconLevel(japUnit, 0, unitParty)
			
		elseif japUnit.sighted and not japUnit.smoked then
		
			ClearForcedReconLevel(japUnit, invParty)
	...
	--if Mission.Realistic then
		...
		if table.getn(Mission.ActiveSmokes) > 0 then
			
			--MissionNarrative("smoke is thinkin!")
			
			Mission.ShipsInSmoke = {}
			
			for idx,smoke in pairs(Mission.ActiveSmokes) do
				
				--local smokePos = GetPosition(smoke)
				--local smokeParty = smoke.party
				local smokeRadius = Mission.ActiveSmokeRadii[idx]
				local smokePos = Mission.ActiveSmokeCoords[idx]
				
				local shipsInThisSmoke = luaGetShipsAroundCoordinate(smokePos, smokeRadius, smoke.party, "own")
				
				--[[if shipsInThisSmoke then
				
					for idx,unit in pairs(luaRemoveDeadsFromTable(shipsInThisSmoke)) do
					
						if table.getn(luaRemoveDeadsFromTable(Mission.ShipsInSmoke)) > 0 then
							
							local tracker = 0
							
							for idx2,unit2 in pairs(luaRemoveDeadsFromTable(Mission.ShipsInSmoke)) do
							...
							if tracker == (table.getn(luaRemoveDeadsFromTable(shipsInThisSmoke)) - 1) then
							
								table.insert(Mission.ShipsInSmoke, unit)
				...
				Mission.ShipsInSmoke = luaSumTablesIndex(Mission.ShipsInSmoke, shipsInThisSmoke)
				
				-- TEST
				
				--[[if shipsInThisSmoke then
				
					Mission.TEST = table.getn(shipsInThisSmoke)
					
					MissionNarrative("ships in smoke! ships present: #Mission.TEST#")
			...
			if Mission.ShipsInSmoke then
				
				if table.getn(luaRemoveDeadsFromTable(Mission.ShipsInSmoke)) > 0 then
				
					for i,unit in pairs(luaRemoveDeadsFromTable(Mission.ShipsInSmoke)) do
					
						if not unit.smoked then
							
							local unitParty = unit.Party
							local invParty = nil
							...
							SetForcedReconLevel(unit, 0, invParty)
							--SetForcedReconLevel(unit, 0, unitParty)
							
							table.insert(Mission.SmokedShips, unit)
							
							unit.smoked = true
							
							-- TEST
							
							--Mission.TEST = unit.Class.Name
							--MissionNarrative("Unit: #Mission.TEST# smoked!")
				...
				for i,unit in pairs(luaRemoveDeadsFromTable(Mission.SmokedShips)) do
					
					unit.canStayInSmoke = false
					--unit.smokeNum = 0
					...
					--if Mission.ShipsInSmoke then
						...
						if unit.smoked then
							...
							for g,smokedUnit in pairs(luaRemoveDeadsFromTable(Mission.ShipsInSmoke)) do
							
								if unit == smokedUnit then
									
									if not unit.canStayInSmoke then
									
										unit.canStayInSmoke = true
									
									end
									
									--unit.smokeNum = unit.smokeNum + 1
							...
							if not unit.canStayInSmoke then
								...
								ClearForcedReconLevel(unit, invParty)	
					
								unit.canStayInSmoke = false
								--unit.smokeNum = 0
								unit.smoked = false
								
								table.remove(Mission.SmokedShips, i)
				...
				--if not Mission.SmokedShipsCleared then
				
					luaClearAllSmokedShips()
			...
			--[[for i,unit in pairs(luaRemoveDeadsFromTable(Mission.SmokedShips)) do
				...
				if unit.smoked and not unit.canStayInSmoke --[[and unit.smokeNum == 0 ]]then
	...
	--[[if Mission.Realistic then
		...
		if not Mission.MissionEnd or not Mission.EndMission then
		
			local usShips = luaGetShipsAroundCoordinate({["x"] = 0, ["y"] = 0, ["z"] = 0}, 400000, PARTY_ALLIED, "own")
			local japShips = luaGetShipsAroundCoordinate({["x"] = 0, ["y"] = 0, ["z"] = 0}, 400000, PARTY_JAPANESE, "own")
			local ships
			
			if usShips and japShips then
			
				ships = luaSumTablesIndex(usShips, japShips)
			
			elseif usShips and not japShips then
				
				ships = usShips
				
				
			elseif not usShips and japShips then
			...
			if ships and table.getn(luaRemoveDeadsFromTable(ships)) > 0 then
				...
				for idx,unit in pairs(luaRemoveDeadsFromTable(ships)) do
					...
					if unit and not unit.Dead then
						...
						if not unit.realisticStatsSet then
							
							local torps = GetProperty(unit, "TorpedoStock")
							
							if torps == unit.Class.MaxTorpedoStock then
							
								ShipSetTorpedoStock(unit, unit.Class.MaxTorpedoStock_Realistic)
							
							end
							
							OverrideHP(unit, unit.Class.HP_Realistic)
							SetShipMaxSpeed(unit, unit.Class.MaxSpeed_Realistic)
							
							unit.realisticStatsSet = true
	...
	--[[if Mission.Classic then
		...
		if not Mission.MissionEnd then
			...
			--[[if Mission.CVsToModerate and table.getn(luaRemoveDeadsFromTable(Mission.CVsToModerate)) > 0 then
				
				Mission.CVsToModerate = luaRemoveDeadsFromTable(Mission.CVsToModerate)
				
				for idx,cv in pairs(luaRemoveDeadsFromTable(Mission.CVsToModerate)) do
					
					if cv and not cv.Dead then
					
						local unitSlots = cv.airBaseSlotCount
						
						for i = 1, unitSlots do
						
							local planeSquad = thisTable[tostring(GetProperty(cv, "slots")[i].squadron)]
							
							if planeSquad and not planeSquad.slottedInAirbase then
								
								local armament = GetProperty(planeSquad, "ammoType")
								
								SetAirBaseSlot(cv, i, planeSquad.Class.ID, 3, armament)
								
								planeSquad.slottedInAirbase = true
								
								Mission.TEST1 = planeSquad.Class.Name
								Mission.TEST2 = planeSquad.Class.ID
								Mission.TEST3 = cv.Class.Name
								Mission.TEST4 = armament
								Mission.TEST5 = i
								MissionNarrative("#Mission.TEST1# (class number #Mission.TEST2#) is set as a slot number #Mission.TEST5# for #Mission.TEST3# with armament of #Mission.TEST4#")
			...
			--SetAirBaseSlot(FindEntity("Shokaku"), 2, 150, 3, 0)
			
			--[[local usPlanes = luaGetPlanesAroundCoordinate({["x"] = 0, ["y"] = 0, ["z"] = 0}, 400000, PARTY_ALLIED, "own")
			local japPlanes = luaGetPlanesAroundCoordinate({["x"] = 0, ["y"] = 0, ["z"] = 0}, 400000, PARTY_JAPANESE, "own")
			local planes
			
			if usPlanes and japPlanes then
			
				planes = luaSumTablesIndex(usPlanes, japPlanes)
			
			elseif usPlanes and not japPlanes then
				
				planes = usPlanes
				
				
			elseif not usPlanes and japPlanes then
			...
			if planes and table.getn(luaRemoveDeadsFromTable(planes)) > 0 then
				...
				for idx,unit in pairs(luaRemoveDeadsFromTable(planes)) do
					...
					if unit and not unit.Dead then
						...
						if not unit.cringeLord then
						
							SetInvincible(unit, 0.01)
							
							MissionNarrative("plane made invincible")
							
							unit.cringeLord = true
	...
	--Mission.TEST = table.getn(Mission.SmokedShips)
	--MissionNarrative("smoked ships: #Mission.TEST#")
	
end

---- LINE OF SIGHT SYSTEM ----

--[[function luaUSUnitSighted(dummyEnt,usUnit)

	if usUnit and not usUnit.Dead then
		
		MissionNarrative("US unit sighted!")
...
function luaJapUnitSighted(dummyEnt,japUnit)

	if japUnit and not japUnit.Dead then
		
		MissionNarrative("Jap unit sighted!")
		
		japUnit.sighted = true
...
function luaAddSmokeScreenListener()

	AddListener("input", "smokeScreenInputListener", {
		["callback"] = "luaSmokeScreenButtonPressed",
		["inputID"] = {IC_CONSOLE_ONOFF},
	})

end

function luaSmokeScreenButtonPressed()
	
	local selUnit = GetSelectedUnit()
	
	if selUnit.Class.SmokeScreen and not selUnit.SmokeCooldown then
	
		luaDropSmoke(selUnit)
		
		selUnit.SmokeCooldown = true
		
	elseif selUnit.SmokeCooldown then
	
		--MissionNarrative("unit already smoked! wait a lil bit broskiskiski!")
		
	else
	
		--MissionNarrative("no can simok!")
	
	end
	
end

function luaDropSmoke(unit)

	--MissionNarrative("smoke function started!")
	
	local smokeDuration = unit.Class.SmokeScreenDuration
	local smokeDropTime = unit.Class.SmokeScreenSpawnTime
	local smokeCooldown = unit.Class.SmokeScreenCooldown --[[+ smokeDropTime]]
	local smokeInterval = (smokeDropTime / 5)
	local smokeRadius = unit.Class.SmokeScreenRadius
	local smokeTracker = smokeInterval
	local smokeSpawnTime = 0
	--local smokeSetToReEnable = false
	
	local shipSmokePos = {["x"]=0, ["y"] = unit.Class.Height+3, ["z"]=0}
	local shipSmoke = Effect("SmokescreenShipSmoke", shipSmokePos, unit, 20)
	
	while (smokeTracker > 0) do
		
		if unit and not unit.Dead then
		
			local smokePos = GetPosition(unit)
			
			--table.insert(smokeScreenPositions, smokePos)
			
			luaDelaySet("UnitToSmoke", unit, smokeSpawnTime)
			luaDelaySet("CurrentSmokeDuration", smokeDuration, smokeSpawnTime)
			luaDelay(luaSpawnSmoke, smokeSpawnTime)
			
			if smokeTracker == smokeInterval then
				
				luaDelaySet("ShipSmokeToRemove", shipSmoke, smokeDropTime)
				luaDelay(luaSmokeDropOver, smokeDropTime)
				
				--MissionNarrative("smoke drop over function called!")
				
			elseif smokeTracker == 1 then
				
				luaDelaySet("ShipToReEnableSmoke", unit, smokeCooldown)
				luaDelay(luaReEnableSmoke, smokeCooldown)
				
				--smokeSetToReEnable = true
				
				--MissionNarrative("smoke re-enable function called!")
				
			end
			
			smokeSpawnTime = smokeSpawnTime + 5
			smokeTracker = smokeTracker - 1
	...
	--MissionNarrative("smoke down!")

end

function luaSpawnSmoke()
	
	--local smokePosTableNum = table.getn(smokeScreenPositions)
	--local pos = smokeScreenPositions[smokePosTableNum]
	
	if Mission.UnitToSmoke and not Mission.UnitToSmoke.Dead then
	
		local efxList = {"Smokescreen_300m"}
		local pos = GetPosition(Mission.UnitToSmoke)
		local radius = Mission.UnitToSmoke.Class.SmokeScreenRadius
		local efx = nil
		...
		if radius == 300 then
			...
			MissionNarrative("that smoke radius has not been set-up yet!")
			
			return
		
		end
		
		local smoke = Effect(efx, pos)
		smoke.party = Mission.UnitToSmoke.Party
		
		table.insert(Mission.ActiveSmokes, smoke)
		table.insert(Mission.ActiveSmokeRadii, radius)
		table.insert(Mission.ActiveSmokeCoords, pos)
		
		luaDelaySet("SmokeToRemove", smoke, Mission.CurrentSmokeDuration)
		luaDelay(luaRemoveSmoke, Mission.CurrentSmokeDuration)
	
	end
	
	-- TEST
	
	--Mission.TEST = table.getn(Mission.ActiveSmokes)
	
	--MissionNarrative("smoke spawned! smokes active: #Mission.TEST#")

end

function luaRemoveSmoke()
	
	--local Mission.SmokeToRemove = smokeScreens[1]
	
	StopEffect(Mission.SmokeToRemove.Pointer)
	
	local idxToRemove = nil
	
	for idx,smoke in pairs(Mission.ActiveSmokes) do
	
		if smoke == Mission.SmokeToRemove then
	...
	table.remove(Mission.ActiveSmokes, idxToRemove)
	table.remove(Mission.ActiveSmokeRadii, idxToRemove)
	table.remove(Mission.ActiveSmokeCoords, idxToRemove)
	
	--table.remove(smokeScreenPositions, 1)
	
	-- TEST
	
	--Mission.TEST = table.getn(Mission.ActiveSmokes)
	
	--MissionNarrative("smoke removed! smokes active: #Mission.TEST#")

end

function luaSmokeDropOver()

	StopEffect(Mission.ShipSmokeToRemove.Pointer)

end

function luaReEnableSmoke()
	
	--local unit = Mission.ShipToReEnableSmoke
	
	if Mission.ShipToReEnableSmoke and not Mission.ShipToReEnableSmoke.Dead then
	
		Mission.ShipToReEnableSmoke.SmokeCooldown = false
	
	end
	
	-- TEST
	
	--Mission.TEST = Mission.ShipToReEnableSmoke.Class.Name
	
	--MissionNarrative("smoke re-enabled for: #Mission.TEST#")

end

function luaClearAllSmokedShips()
	
	if table.getn(luaRemoveDeadsFromTable(Mission.SmokedShips)) > 0 --[[and not Mission.SmokedShipsCleared]]then
	
		for i,unit in pairs(luaRemoveDeadsFromTable(Mission.SmokedShips)) do
		
			--if not unit.cleared then
			...
			if unit and not unit.Dead then
				...
				--Mission.TEST = unit.Class.Name
				--MissionNarrative("Unit: #Mission.TEST# no longer smoked!")
		...
		--Mission.SmokedShipsCleared = true
...
function luaSetUnlockName(unit)
	
	if not Mission.UnlockNameTable then
	
		Mission.UnlockNameTable = {
	...
	if unit and not unit.Dead then
		...
		if IsClassChanged(unit.ClassID) then
			...
			local classNum = unit.Class.ID
			local nameTable = Mission.UnlockNameTable[classNum]
			
			if nameTable and table.getn(nameTable) > 0 then
				
				local nameIdx = luaRnd(1, table.getn(nameTable))
				local name = nameTable[nameIdx]
				
				SetGuiName(unit, name)
				
				table.remove(nameTable, nameIdx)
...
--[[function luaABStock1Launched()
	
	local selUnit = GetSelectedUnit()
	
	if selUnit and not selUnit.Dead then
		
		luaLaunchSquad(1, selUnit.slot1Stock)
		
	end
	
end

function luaABStock2Launched()

	local selUnit = GetSelectedUnit()
	
	if selUnit and not selUnit.Dead then
		
		luaLaunchSquad(2, selUnit.slot2Stock)
		
	end

end

function luaABStock3Launched()

	local selUnit = GetSelectedUnit()
	
	if selUnit and not selUnit.Dead then
		
		luaLaunchSquad(3, selUnit.slot3Stock)
		
	end

end

function luaABStock4Launched()

	local selUnit = GetSelectedUnit()
	
	if selUnit and not selUnit.Dead then
		
		luaLaunchSquad(4, selUnit.slot4Stock)
		
	end

end

function luaABStock5Launched()

	local selUnit = GetSelectedUnit()
	
	if selUnit and not selUnit.Dead then
		
		luaLaunchSquad(5, selUnit.slot5Stock)
		
	end

end

function luaABStock6Launched()

	local selUnit = GetSelectedUnit()
	
	if selUnit and not selUnit.Dead then
		
		luaLaunchSquad(6, selUnit.slot6Stock)
		
	end

end

function luaABStock7Launched()

	local selUnit = GetSelectedUnit()
	
	if selUnit and not selUnit.Dead then
		
		luaLaunchSquad(7, selUnit.slot7Stock)
		
	end

end

function luaABStock8Launched()

	local selUnit = GetSelectedUnit()
	
	if selUnit and not selUnit.Dead then
		
		luaLaunchSquad(8, selUnit.slot8Stock)
		
	end

end

function luaLaunchSquad(slotIdx, stock)
	
	MissionNarrative("squadron launch called!")
	
	local selUnit = GetSelectedUnit()
	
	if selUnit and not selUnit.Dead then
		
		if selUnit.Class.Type == "MotherShip" or selUnit.Class.Type == "AirField" then
			
			if not stock == nil then
				
				local armament
				
				if slotIdx <= selUnit.airBaseSlotCount then
					
					if GetAirBaseSlotStatus(selUnit, slotIdx) == 0 then
					
						SetAirBaseSlot(selUnit, slotIdx, stock, 3, 0)
						LaunchSquadron(selUnit, stock, 3)
						
						MissionNarrative("squadron launched!")
						
					else
					
						MissionNarrative("slot already in use bruv!")
					
					end
					
				else
				
					MissionNarrative("no slot there bruv!")
				
				end
			
			else
			
				MissionNarrative("no stock there bruv!")
			
			end
			
		else
		
			MissionNarrative("unit not an airbase bruv!")
...
function luaGetMPPlayers()
	...
	for i = 0,7 do
		...
		if GetValidPlayerInSlot(i) == true then
...
function luaCheckSkirmish()
	
	if not Mission.SkirmishDetermined then
	
		local players, playerIdx = luaGetMPPlayers()
		
		if players == 1 then
		
			Mission.Skirmish = true
			
			Mission.SkirmishAIUnits = {}
			Mission.SkirmishAICVs = {}
			Mission.SkirmishOppositeShips = {}
			
			local skirmishPlayerIdx = playerIdx
			local checkDistance = 9000000
			
			local japShips = luaGetShipsAroundCoordinate({["x"] = 0, ["y"] = 0, ["z"] = 0}, checkDistance, PARTY_JAPANESE, "own")
			local usShips = luaGetShipsAroundCoordinate({["x"] = 0, ["y"] = 0, ["z"] = 0}, checkDistance, PARTY_ALLIED, "own")
			
			if skirmishPlayerIdx <= 3 then
			
				Mission.SkirmishAI = 1
				
				if japShips and table.getn(luaRemoveDeadsFromTable(japShips)) > 0 then
				
					for idx,unit in pairs(luaRemoveDeadsFromTable(japShips)) do
						
						if unit.Class.Type == "MotherShip" or unit.Class.Type == "AirField" then
							
							unit.LaunchPlaneParty = 1
							
							table.insert(Mission.SkirmishAICVs, unit)
						
						else
							
							table.insert(Mission.SkirmishAIUnits, unit)
					...
					if Mission.CVsToModerate and table.getn(luaRemoveDeadsFromTable(Mission.CVsToModerate)) > 0 then
						...
						for idx,unit in pairs(luaRemoveDeadsFromTable(Mission.CVsToModerate)) do
					...
					if usShips and table.getn(luaRemoveDeadsFromTable(usShips)) > 0 then
					
						Mission.SkirmishOppositeShips = usShips
					
					end
					
				end
				
			elseif skirmishPlayerIdx >= 4 then
			
				Mission.SkirmishAI = 0
				
				if usShips and table.getn(luaRemoveDeadsFromTable(usShips)) > 0 then
				
					for idx,unit in pairs(luaRemoveDeadsFromTable(usShips)) do
						
						if unit.Class.Type == "MotherShip" or unit.Class.Type == "AirField" then
							
							unit.LaunchPlaneParty = 0
					...
					if japShips and table.getn(luaRemoveDeadsFromTable(japShips)) > 0 then
						...
						Mission.SkirmishOppositeShips = japShips
		...
		Mission.SkirmishDetermined = true
	
	elseif Mission.Skirmish then
		
		local aiCvTrgs
		
		if Mission.SkirmishAI == 0 then
		
			aiCvTrgs = luaRemoveDeadsFromTable(Mission.IJNKeyUnits)
		
		elseif Mission.SkirmishAI == 1 then
		
			aiCvTrgs = luaRemoveDeadsFromTable(Mission.USNKeyUnits)
		
		end
		
		if table.getn(luaRemoveDeadsFromTable(Mission.SkirmishAIUnits)) > 0 then
		
			for idx,leader in pairs(luaRemoveDeadsFromTable(Mission.SkirmishAIUnits)) do
			
				if leader and not leader.Dead then
				
					if not leader.stanceSet then
						
						if table.getn(luaRemoveDeadsFromTable(Mission.SkirmishOppositeShips)) > 0 then
						
							NavigatorAttackMove(leader, luaPickRnd(luaRemoveDeadsFromTable(Mission.SkirmishOppositeShips)))
							
						end
						
						UnitSetFireStance(leader, 2)
						
						leader.stanceSet = true
		...
		if table.getn(luaRemoveDeadsFromTable(Mission.SkirmishAICVs)) > 0 then
		
			for idx,cv in pairs(luaRemoveDeadsFromTable(Mission.SkirmishAICVs)) do
			
				if cv and not cv.Dead then
					
					aiCvTrgs = luaRemoveDeadsFromTable(aiCvTrgs)
					
					if aiCvTrgs and table.getn(luaRemoveDeadsFromTable(aiCvTrgs)) > 0 then
						
						local fighterClass
						local bomberClass
						
						if cv.LaunchPlaneParty == 0 then
						...
						elseif cv.LaunchPlaneParty == 1 then
						...
						luaAirfieldManager(cv, fighterClass, bomberClass, aiCvTrgs)
...
--[[function luaRedistributeMPUnits(sweep, usArg, japArg)
	...
	if sweep then
		...
		usUnits = luaSumTablesIndex(usShips, usPlanes)
		japUnits = luaSumTablesIndex(japShips, japPlanes)
	...
	local validPlayers = {}
	local validUsPlayers = {}
	local validJapPlayers = {}
	local usUnOwnedUnits = {}
	local japUnOwnedUnits = {}
	
	for i = 0,7 do
	
		if GetValidPlayerInSlot(i) == false then
		
			table.insert(invalidPlayers, i)
		
		else
		
			table.insert(validPlayers, i)
		
		end
	
	end
	
	if table.getn(invalidPlayers) > 0 then
		
		for idx,validPlayerIdx in pairs(validPlayers) do
			
			if validPlayerIdx <= 3 then
			
				table.insert(validUsPlayers, validPlayerIdx)
			
			elseif validPlayerIdx >= 4 then
			
				table.insert(validJapPlayers, validPlayerIdx)
			
			end
		
		end
		
		for abc,invalidPlayerIdx in pairs(invalidPlayers) do
			
			if invalidPlayerIdx <= 3 then
				
				for idx,unit in pairs(usUnits) do
					
					local unitRole = GetRoleAvailable(unit)[1]
					
					if unitRole == invalidPlayerIdx then
						
						table.insert(usUnOwnedUnits, unit)
						
					end
				
				end
				
			elseif invalidPlayerIdx >= 4 then
			
				for idx,unit in pairs(japUnits) do
					
					local unitRole = GetRoleAvailable(unit)[1]
					
					if unitRole == invalidPlayerIdx then
						
						table.insert(japUnOwnedUnits, unit)
		...
		if table.getn(validUsPlayers) > 0 then
		
			for idx,unit in pairs(usUnOwnedUnits) do
				
				SetRoleAvailable(unit, EROLF_ALL, luaPickRnd(validUsPlayers))
				
			end
		
		end
		
		if table.getn(validJapPlayers) > 0 then
		
			for idx,unit in pairs(japUnOwnedUnits) do
				
				SetRoleAvailable(unit, EROLF_ALL, luaPickRnd(validJapPlayers))
